...
class InventoryServiceTest {
	...
	void filterApplesTest_lambda() {
		...
		List<Apple> result = inventoryService.filterApples(inventory, (Apple a) -> Color.GREEN.equals(a.getColor()));
		assertEquals(1, result.size());
		assertEquals(Color.GREEN, result.get(0).getColor());

		List<Apple> result2 = inventoryService.filterApples(inventory, (Apple a) -> a.getWeight() > 150);
		assertEquals(1, result2.size());
		assertEquals(152d, result2.get(0).getWeight());
